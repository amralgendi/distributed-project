...
class RMQEventReceiver(threading.Thread):
    def __init__(self, events: List[Event], did_receive_function):
        super().__init__()
        self.events = events
    ...
    def consume_messages(self, events: List[Event]):
        try:
            # Establish a new connection
            self.connection = pika.BlockingConnection(pika.ConnectionParameters(RMQ_ADDRESS, port=RMQ_PORT))
    ...
    def callback(self, ch, method, properties, body):
        ...
        if event_name in Event.__members__:
            message_enum = Event[event_name]
